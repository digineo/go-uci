...
type Tree interface {
	...
	Set(config, section, option string, values ...string) error
...
func (t *tree) Set(config, section, option string, values ...string) error {
	if len(values) > 1 {
		err := t.SetType(config, section, option, TypeList, values...)
		if err != nil {
			return fmt.Errorf("t.SetType: %w", err)
		}
	}
	err := t.SetType(config, section, option, TypeOption, values...)
	if err != nil {
		return fmt.Errorf("t.SetType: %w", err)
	}
	return nil
